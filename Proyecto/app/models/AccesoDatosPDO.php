...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    public function getClientesOrdenados($ordenarPor, $primero, $cuantos): array {
        $tuser = [];
        
        // Validar el campo de ordenación para evitar inyecciones SQL
        $validFields = ['id', 'first_name', 'last_name', 'email', 'gender', 'ip_address'];
        if (!in_array($ordenarPor, $validFields)) {
            $ordenarPor = 'id'; // Valor por defecto
        }
    
        $stmt_usuarios = $this->dbh->prepare("SELECT * FROM Clientes ORDER BY $ordenarPor LIMIT :primero, :cuantos");
    ...
    { 
     ...
     public function emailExists($email, $id) {
        ...
        $query = "SELECT COUNT(*) FROM clientes WHERE email = :email AND id != :id";
